...
description: >
  ...
  fetch their actual code, analyze patterns, and produce a structured fusion report. Use before
  creating or improving a skill when you want to see what already exists — "find skills for X",
  "survey the ecosystem", "find similar skills", "what's out there". Produces
  a fusion report for skill-creator to consume. Do NOT use for installing skills or general research.
version: 0.1.0
---
...
This skill handles the **research phase** of skill creation. After producing the fusion report, hand off to `skill-creator` for writing, evaluating, and optimizing the actual SKILL.md.
...
- User wants to search for a skill to install (not to analyze for fusion) — use `npx skills find`
...
|------|-------------------|----------------|----------|
| **Quick** | 5–8 skills | Key patterns + comparison table | Narrow topic, time-sensitive, or the user already knows the landscape |
...
**Do not assume a mode.** If the user hasn't specified Quick, Standard, or Deep, ask them which mode they want before proceeding. Briefly explain the trade-off: more candidates = richer fusion material but longer research time. Let the user decide.
...
Once topic, intent, and depth are confirmed, announce the search plan: "I'll search for community skills related to [topic] across [N sources]. Target: [mode] mode, [N] candidates."
...
| Source | How to Check |
|--------|-------------|
| **awesome-claude-code** (32.7k stars) | `WebFetch https://github.com/hesreallyhim/awesome-claude-code` and search for topic |
| **awesome-agent-skills** (12.9k stars) | `WebFetch https://github.com/VoltAgent/awesome-agent-skills` |
...
| **alirezarezvani/claude-skills** (192+ skills) | `WebFetch https://github.com/alirezarezvani/claude-skills` |
...
**Wait for user confirmation before Phase 3.** This is the key checkpoint — the user may know about sources you missed, or want to exclude low-quality candidates.
...
Produce the **Fusion Report** — the deliverable of this skill.
...
3. If the user wants to proceed immediately, summarize the key decisions from the fusion report as context for skill-creator
