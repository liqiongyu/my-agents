...
### 7.3 完整目标架构中的 Reference Capability Families

本章前面定义的是边界规则。
但完整目标架构仍然必须明确回答一件事：
**最终整套系统原则上要具备哪些 capability families。**

这里给出的不是“今天就要全部实例化的 package backlog”，而是完整实施形态下应被系统明确建模的能力家族。
其中有些 family 更接近 agent-capable family，有些更接近 service-capable family 或长期保留在 skill/policy 层。

#### Producer / Intake Families

这些家族负责把不同来源的原始信号转成可治理的 issue 输入：

- `Feedback Intake Normalizer`
  - 把用户反馈、外部线索转成标准 issue 草案
- `Review Findings Normalizer`
  - 把 review 发现、评审意见转成待治理 issue
- `Bug Signal Normalizer`
  - 把异常、日志、缺陷线索转成 bug issue
- `Roadmap Candidate Synthesizer`
  - 把规划性目标或长期方向转成候选工作项
...
- `Shaper / Decomposer`
  - 负责 issue 整形、边界收敛、拆分 parent / child issue
- `Admission / Triage Coordinator`
  - 负责准入、优先级、风险和排序
- `Run Orchestrator`
  - 负责创建 run、回收 run、重试、升级和终止
- `Projection / State Sync Operator`
  - 负责把 canonical 对象映射到 GitHub 等外部表面
- `Learn / Backlog Curator`
  - 负责失败模式提炼、follow-up issue、记忆写回和 backlog 卫生

#### Issue Cell Families

这些家族负责单 issue 的核心执行闭环：

- `Case Owner`
  - 持有当前 issue 的执行策略和上下文
- `Builder`
  - 推进核心实现、测试和修订
- `Critic`
  - 对实现结果做对抗性评审
- `Verifier`
  - 对 done contract、验证证据和 gate 条件做形式化确认
- `Closer`
  - 负责 merge / close / recycle 的收口动作

#### Support / Advisory Families

这些家族在 issue cell 内按需提供辅助能力：

- `Explore Utility`
  - 代码库结构侦察、未知依赖定位、影响面分析
- `Planning Utility`
  - 当前 run 的局部计划、步骤拆解、预算内重排
- `Diff / Impact Analyst`
  - 分析当前变更的扩散风险和测试建议
- `Trace Summarizer`
  - 压缩长链路执行历史，生成可消费摘要
- `Learn Synthesizer`
  - 把单 run 的经验沉淀成更长期的 failure pattern 或 follow-up
...
这些 specialist families 的存在，是完整目标架构的一部分。
是否在某一阶段把它们全部实例化，是 rollout 问题，不是系统本体问题。

### 7.4 实施期的默认收缩形态

完整目标架构并不等于默认每次运行都“角色全开”。
在实施期或第一阶段落地中，推荐的默认 issue runtime 形态是：
...
| `Feedback Intake Normalizer`       | intake skill 或 issue agent       | 否             | 需要多来源独立 intake 上下文时              | normalization skill         |
| `Review Findings Normalizer`       | intake skill 或 issue agent       | 否             | 需要批量吸收 review 发现并归档时            | normalization skill         |
| `Bug Signal Normalizer`            | intake skill 或 issue agent       | 否             | 需要持续分析异常/缺陷线索时                 | normalization skill         |
| `Roadmap Candidate Synthesizer`    | shaping skill 或 planner          | 否             | 需要长期规划线程独立运行时                  | shaping skill               |
| `Shaper / Decomposer`              | shaping path 或专职 agent         | 否             | 需要高频拆分、独立 issue-graph 判断时       | shaping skill / service     |
| `Admission / Triage Coordinator`   | control-plane function            | 否             | 需要独立治理队列与预算上下文时              | policy + service            |
| `Run Orchestrator`                 | control-plane service             | 否             | 原则上不应人格化                            | service                     |
| `Projection / State Sync Operator` | adapter/service                   | 否             | 需要独立平台同步上下文时                    | service                     |
| `Learn / Backlog Curator`          | learn service 或 curation agent   | 否             | 需要独立长期学习与 backlog 治理线程时       | service / writeback skill   |
...
| `Product / Acceptance Specialist`  | specialist skill 或 advisory path | 否             | 需要独立需求边界和验收裁决上下文时          | shaping / acceptance skill  |
| `Design / UI Specialist`           | specialist skill 或 agent         | 否             | 需要独立设计判断和视觉证据采集时            | design review skill         |
| `Security Specialist`              | gate-bound specialist agent       | 否             | 需要独立安全 gate 或敏感权限边界时          | security review skill       |
| `Performance Specialist`           | specialist skill 或 agent         | 否             | 需要独立 profiling / benchmark 上下文时     | performance review skill    |
| `Data Migration Specialist`        | gate-bound specialist agent       | 否             | 需要独立迁移计划、数据检查和回滚判断时      | migration planning skill    |
| `Release / Infra Specialist`       | specialist skill 或 agent         | 否             | 需要独立发布、环境或 CI/CD 权限边界时       | release readiness skill     |

### 7.6 哪些家族在完整目标架构中通常值得保留为 Agent

如果谈的是**最终完整实施后的整套系统**，通常值得长期保留为独立 agent 或 agent-capable family 的，不止三类，而是至少以下几组：
...
- `Evaluator / Critic`
  - 负责与主执行单元形成质量对抗或独立评估
- `Shaper / Decomposer`
  - 在 issue graph 复杂、拆分频繁的系统中经常值得独立持有上下文
- `Producer / Intake Families`
  - 当系统真的从多来源持续探索需求时，这些 family 会成为长期存在的输入前端
- `Tool-Bound or Gate-Bound Specialists`
  - 例如 `Browser QA Specialist`、`Architecture Specialist`、`Security Specialist`
- `Learn / Backlog Curator`
  - 当系统长期运行且需要持续回写经验时，可能值得独立化

这代表的是完整目标架构的可实现上界，而不是所有部署都必须一次性全部实例化。

### 7.7 更适合降级为非-Agent的内容
...
### 8.5 完整目标架构中的 Reference Specialist Catalog

为了让完整系统蓝图具备可实施性，建议至少明确以下 specialist families：

| Specialist Family                 | 主要职责                           | 典型工具域                                | 常见介入方式          | 典型输出                                |
| --------------------------------- | ---------------------------------- | ----------------------------------------- | --------------------- | --------------------------------------- |
| `Browser QA Specialist`           | 浏览器回归、真实交互验证、截图证据 | browser automation、DOM、console、network | advisory / gate       | browser QA evidence, regression verdict |
| `Architecture Specialist`         | 边界约束、接口设计、跨模块一致性   | repo graph、ADR、dependency analysis      | advisory / gate       | architecture review, boundary decision  |
| `Product / Acceptance Specialist` | 验收标准、非目标、需求边界强化     | issue shaping、acceptance rubric          | advisory / escalation | refined acceptance criteria             |
| `Design / UI Specialist`          | 设计系统一致性、界面和交互判断     | design system、visual diff、UI tools      | advisory / gate       | design review, UI decision              |
| `Security Specialist`             | 风险识别、安全 gate、敏感变更检查  | security tooling、threat checklist        | gate / escalation     | security verdict, risk note             |
| `Performance Specialist`          | 性能热点、回归、预算影响           | profiling、benchmark、perf traces         | advisory / gate       | perf report, budget impact              |
| `Data Migration Specialist`       | schema 变更、迁移策略、数据一致性  | migration tooling、DB inspection          | advisory / gate       | migration plan, consistency decision    |
| `Release / Infra Specialist`      | 部署、CI/CD、环境、发布约束        | CI systems、deploy tooling、infra context | advisory / gate       | release readiness, infra note           |

这份目录表达的是完整目标架构中专家层应覆盖的能力面。
某个部署是否全部启用它们，是后续实施策略问题。

### 8.6 Governance Family Crosswalk

治理相关 family 在蓝图中会以不同层级出现：

- 逻辑角色或 agent-capable family
- service family
- skill family

为了避免把这些不同层级误读成多套无关组件，下面给出建议 crosswalk：

> **Note:** Skill / Policy 列中列出的是逻辑能力名称。部分曾作为独立 skill 包实现的
> bridge-era governance skills（如 `admission-routing`、`priority-scoring` 等）
> 已在迁移到 skill-based 架构时移除。这些名称仍作为能力域参考保留。

| 能力域                  | Agent / Role 视角                  | Service 视角                            | Skill / Policy 视角                                                     |
| ----------------------- | ---------------------------------- | --------------------------------------- | ----------------------------------------------------------------------- |
| Admission / Triage      | `Admission / Triage Coordinator`   | `Admission / Triage Service`            | `admission-routing`, `priority-scoring`, `risk-gating`                  |
| Run governance          | `Run Orchestrator`                 | `Run Manager`                           | `budget-decision`, escalation policy                                    |
| Projection              | `Projection / State Sync Operator` | `Projection / State Sync Adapter Layer` | `projection-sync`, `artifact-linking`                                   |
| Learn / curation        | `Learn / Backlog Curator`          | `Memory / Learn Service`                | `trace-summarization`, `failure-pattern-extraction`, `backlog-curation` |
| Shaping / decomposition | `Shaper / Decomposer`              | `Shaping / Decomposition Service`       | `issue-shaping`, `decomposition-planning`                               |

这张表的目的不是强行统一所有命名。
它的目的是明确：

- 这些通常是在表达同一能力域的不同物化层
- 不是在暗示必须存在多套彼此独立的运行组件
...
### 9.11 完整目标架构中的 Reference Skill Families

为了让完整系统蓝图真正能指导实施，建议长期维护一份参考 skill 目录。
下面的 skill families 代表的是**完整系统应覆盖的能力面**，不是今天就必须全部建包。

> **Note:** 部分 skill 名称曾作为独立包实现（bridge-era governance skills），
> 已在迁移到 skill-based 架构后移除。下列名称现仅作为逻辑能力参考。
> 当前已实现的 skill 请查看 `docs/catalog/skills.md`。

#### Producer / Intake Skills

- `feedback-normalization`
- `review-finding-normalization`
- `bug-signal-normalization`
- `roadmap-candidate-synthesis`

#### Shaping / Governance Skills
...
#### Evaluation / Verification Skills
...
这些 skill names 是参考能力名，不要求与当前 repo 中现有目录一一对应。
它们表达的是完整系统在方法层、工作流层应该具备哪些能力。

### 9.12 完整目标架构中的 Reference Context Pack Families

完整目标架构不仅需要 agent、skill、service，也需要稳定的 context pack families。
至少应明确以下上下文家族：

- `Product / Acceptance Context Pack`
- `Architecture / ADR Context Pack`
- `Design System Context Pack`
- `Security / Risk Context Pack`
- `Performance / Budget Context Pack`
- `Migration / Data Integrity Context Pack`
- `Release / Infra Context Pack`
- `Repo Exploration Context Pack`
- `Failure Pattern Context Pack`

这些 context pack families 解决的不是“谁执行”，而是“某类能力在判断或执行时默认携带哪些背景材料”。

### 9.13 完整目标架构中的 Reference Service Families

完整实施后的系统不只需要 agent 和 skill，还需要具体的 service families。
至少应明确以下服务面：

- `Canonical Issue Graph Service`
- `Shaping / Decomposition Service`
- `Admission / Triage Service`
- `Run Manager`
- `Specialist Registry / Router`
...
- `Workspace / Sandbox Manager`
- `Trace & Artifact Store`
- `Projection / State Sync Adapter Layer`
- `Memory / Learn Service`

这些 service families 不是可选配角。
它们是完整系统真正能持续运行的基础设施面。

### 9.14 Reference Assembly Patterns

为了避免“有目录，但不知道怎么装配”，完整目标架构还应有基础装配模式。
...
- `Verification & Gate Engine`
- `Run Manager`
- `issue-shaping`, `execution-briefing`, `change-implementation`, `critic-review`, `acceptance-verification`
...
- `Design / UI Specialist`
- `Browser QA Specialist`
- 相关 design / browser QA context packs 与 evidence capture
...
- `Architecture Specialist`
- `Diff / Impact Analyst`
- `architecture-review`, `regression-check`, `risk-gating`
...
- `Data Migration Specialist`
- `Release / Infra Specialist`
- `budget-decision`, `security-review`, `artifact-linking`, `projection-sync`

#### 上游 intake -> shaping -> admission 路径

- `Feedback Intake Normalizer` 或其他 producer / intake family
- `Shaper / Decomposer`
- `Admission / Triage Coordinator`
- `Canonical Issue Graph Service`
- `feedback-normalization`, `issue-shaping`, `decomposition-planning`, `admission-routing`, `priority-scoring`

这条路径表达的是完整系统上游如何把原始信号变成正式可执行 issue。

#### 下游 verification -> learn -> backlog curation 路径

- `Verifier`
- `Learn / Backlog Curator`
- `Memory / Learn Service`
- `Verification & Gate Engine`
- `trace-summarization`, `failure-pattern-extraction`, `follow-up-issue-generation`, `memory-writeback`, `backlog-curation`

这条路径表达的是完整系统下游如何把一次执行结果转成系统级学习、回写和后续工作项。

这些装配模式代表完整系统的常见路径。
实施期可以收缩，但完整蓝图必须先把它们讲全。
